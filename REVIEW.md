...
**Bước 4: Entity giữ vững tính Rich (Tránh bị Anemic):**
Đừng biến Entity thành rỗng. Hãy cho Entity hàm nhận Interface ở dạng đối số (Method Injection).
```python
class Zone:
    def contains_point(self, point: Point, geo_calc: IGeometryCalculator) -> bool:
        return geo_calc.is_point_in_zone(point, self)
```
...
1. **`models.py`**:
   ...
   - **Thay đổi quan trọng:** `Route.stops_seq` trở thành mảng `List[str]` (lưu mảng `stop_id`) để tuân thủ DDD (Entity tham chiếu đến nhau qua ID).
   - **Bổ sung Aggregate Root `TransitNetwork`:** Để tránh việc hàm Routing phải nhận cả chục mảng tham số (list routes, list stops, list zones), hãy tạo một class `TransitNetwork` gom tất cả lại dưới dạng Dictionary (Map) để truy xuất O(1).
     ```python
     class TransitNetwork:
         def __init__(self, stops: dict[str, Stop], routes: dict[str, Route], zones: dict[str, Zone]):
             self.stops = stops
             self.routes = routes
             self.zones = zones
     ...
     ```
     `TransitNetwork` là đối tượng CỦA DOMAIN, hoàn toàn PURE PYTHON. Tầng Adapters (Repository) chỉ có nhiệm vụ query Database để xếp dữ liệu thành cái `TransitNetwork` này rồi giao cho Domain.
2. **`services.py`**:
