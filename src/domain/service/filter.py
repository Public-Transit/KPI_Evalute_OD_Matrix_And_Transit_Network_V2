...
from src.domain.service.spatial import get_closest_stop, get_first_stop_in_route, get_last_stop_in_route
...
class MinDistanceCandidateTripFilterV2(AbstractCandidateTripFilterV2):
    ...
    def filter(self, od_pair: ODPair, od_matrix: ODMatrix, transit_network: TransitNetwork, candidate_trip: CandidateTrip, geometry_calculator: IGeometryCalculator) -> Trip:
        ...
        if len(candidate_trip.candidate_legs) == 1:
            ...
            board_stop = get_closest_stop(leg1.possible_boarding_stop_ids, origin_centroid, transit_network, geometry_calculator)
            alight_stop = get_closest_stop(leg2.possible_alighting_stop_ids, dest_centroid, transit_network, geometry_calculator)
...
class FullTripFilter(AbstractCandidateTripFilterV2):
    def filter(self, od_pair: ODPair, od_matrix: ODMatrix, transit_network: TransitNetwork, candidate_trip: CandidateTrip, geometry_calculator: IGeometryCalculator) -> Trip:
        if not candidate_trip or not candidate_trip.candidate_legs:
            return None
        ...
        if len(candidate_trip.candidate_legs) == 1:
            ...
            board_stop_id = get_first_stop_in_route(leg.possible_boarding_stop_ids, route)
            alight_stop_id = get_last_stop_in_route(leg.possible_alighting_stop_ids, route)
            
            if board_stop_id and alight_stop_id:
                return Trip([Leg(leg.route_id, board_stop_id, alight_stop_id)])
            ...
            board_stop_id = get_first_stop_in_route(leg1.possible_boarding_stop_ids, route1)
            alight_stop_id = get_last_stop_in_route(leg2.possible_alighting_stop_ids, route2)
            
            if not board_stop_id or not alight_stop_id:
                return None
            
            board_stop = transit_network.get_stop_by_id(board_stop_id)
            alight_stop = transit_network.get_stop_by_id(alight_stop_id)
