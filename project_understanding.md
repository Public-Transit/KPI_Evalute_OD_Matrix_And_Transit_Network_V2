...
*   **Results**:
    *   `EvaluatedRoutingOption`: Represents a complete travel option with its raw `CandidateTrip` and selected optimized `Trip`.
    *   `ODRoutingResultV2`: Stores the result of routing for an OD pair, holding a list of `EvaluatedRoutingOption`s.
...
3.  **Filtration & Selection**: The `AbstractCandidateTripFilterV2` filters the candidate trips into optimal ones.
4.  **Result Storage**: The result is wrapped in `ODRoutingResultV2` and saved/passed to the next phase.
5.  **KPI Calculation**: By passing the `EvaluatedRoutingOption`, the KPI Domain Service calculates metrics like the exact transfer rate or circuity index.
...
*   Serve the `ODRoutingResultV2` data structure back as a JSON payload for front-end visualizers or reporting tools.
